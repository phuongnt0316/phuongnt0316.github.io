	...
	<div id="contents">
		<div class="background">
			<div id="blogs">
				...
				<?php
                    $search=$_GET['search'];
					$check=$get_data->check_search_blog($search);
					if($check>0){
                    $se_blog=$get_data->search_blog($search);
					foreach($se_blog as $se){					
					?>
					<li>
						<p class="time"><?php echo $se['Date_blog'];?>|<?php echo $se['Fullname']?></p>
						<h4><?php echo $se['Name_blog'];?></h4>
                        <img src="img_blog/<?php echo$se['image']?>" class="image">
						<p>
							<?php echo $se['S_blog']; ?>
						</p>
						<p>
					...
					echo "<h1>Không có dữ liệu!&emsp;&emsp;&emsp;&emsp;&emsp;&emsp;&emsp;&emsp;</h1>"	
					?>
				</div>
			</div>
		</div>
