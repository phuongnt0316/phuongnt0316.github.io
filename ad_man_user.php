	...
	<div id="header">
		...
		<div id="navigation">
			<ul>
				...
				<li><a href="ad_man_hd.php">Blog header</a></li>
		...
		<div class="background">
            ...
            <h1>User  <a href="register.php" class="icon"><i class='bx bx-user-plus' ></i></a></h1>
				<table>
					...
					?>
					<tr>
						<td><?php echo $se['id_regt']?></td>
						<td><?php echo $se['Fullname']?></td>
                        <td><?php echo $se['email']?></td>
                        <td><?php echo $se['password']?></td>
                        <td><?php echo $se['gender']?></td>
						<?php $name=$get_data->select_nametp($se['address']);
						foreach ($name as $nametp){
						?>
						<td><?php echo $nametp['name'];}?></td>
						<td><?php echo $se['birthday']?></td>
						<td><img src="img/<?php echo $se['file']?>" class="imguser"></td>
						<td><a href="ad_user_update.php?update=<?php echo $se['id_regt']?>" class="icon"><i class='bx bxs-edit-alt' ></i></a></td>
						<td><a href="ad_user_delete.php?delete=<?php echo $se['id_regt']?>" onclick="return (confirm('Are you sure dalete?'))" class="icon"><i class='bx bxs-trash'></i></a></td>
						<td><a href="ad_blog_user.php?user_id=<?php echo $se['id_regt']?>"  class="icon"><i class="fa-solid fa-eye"></i></a></td>
